...
from typing import Any, Dict, List, Optional, Union, Literal, TypedDict, Required, TypeAlias
...
class DeepSeekRequestChat(TypedDict, total=False):
    ...
    - Each message must be an object (dictionary) with the following keys:
        ...
        - name (Optional[str]): An optional name to differentiate between participants with the same role.
    ...
    """
    [Optional] Maximum number of tokens that can be generated for the chat completion.
    ...
    """
    [Optional] One or more sequences where the API will stop generating further tokens.
    ...
    """
    [Optional] Flag indicating whether partial message deltas will be streamed as they become available.
    ...
    """
    [Optional] An object providing options for streaming responses.
    ...
    """
    [Optional] When enabled, an additional chunk containing token usage statistics will be sent
    ...
    """
    [Optional] Sampling temperature to control the randomness of the generated output.
    ...
    """
    [Optional] Nucleus sampling parameter that limits the tokens considered for generation.
    ...
    """
    [Optional] A list of tools that the model may call during response generation.
    ...
    """
    [Optional] When enabled (set to true), the API returns the log probabilities of each generated token.
    - This is useful for analyzing the model's token generation process.
    """

    top_logprobs: Optional[int]
    """
    [Optional] Specifies the number of most likely tokens to return at each token position.
